...
class AndroidConfig(Base):
    ...
    mode: Literal["auto", "adb", "termux"] = "auto"
    # ADB options (used when mode='adb' or auto-detected)
    adb_host: str = "localhost"
    adb_port: int = 5037
    adb_serial: str = ""  # Specific device serial (leave empty for default device)
    # Termux options (used when mode='termux' or running on-device)
    termux_api: bool = True  # Use Termux:API extras if available (termux-screenshot, termux-share)
    screenshots_dir: str = "/sdcard/DCIM/MOBOT"  # Where screenshots are saved on device

class Config(BaseSettings):
    """Root configuration for mobot."""
    ...
    model_config = ConfigDict(env_prefix="MOBOT_", env_nested_delimiter="__")
